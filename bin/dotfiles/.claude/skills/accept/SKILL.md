...
3. **Give Codex its own worktree branched from the task branch.** Per the worktree-per-task rule in CLAUDE.md, Claude works in its own task worktree; Codex implements in a *separate* worktree so this session's tree stays clean for review and the two don't fight over files. Create it once and reuse it for every task in the loop. From the task worktree (its `HEAD` is the task branch), create a child worktree on a new `<task>-impl` branch:
   ...
   ```
   This deliberately stacks `<task>-impl` on the still-unmerged task branch, but only for the duration of the loop: `<task>-impl` is a scratch branch for Codex's edit sandbox, and step 10 fast-forwards it back into the task branch and deletes it as soon as the implementation is done. Never leave the stack open — the task branch, not `<task>-impl`, is what gets reviewed and PR'd to the integration branch.
...
10. **When Codex's implementation is done, fast-forward the task branch and remove Codex's worktree.** Do this as soon as the last task in the list is committed — don't wait for the operator to land the work, and don't leave the impl worktree lying around between accept runs. From Claude's task worktree (the task branch is checked out there):
    ```
    git merge --ff-only <task>-impl
    git worktree remove ../<repo>-<task>-impl
    git branch -d <task>-impl
    ```
    - **Always `--ff-only`, never a plain `git merge`.** `<task>-impl` was branched from the task branch's HEAD and has only gained Claude's reviewed per-task commits on top, so it must fast-forward; a merge commit would put a pointless bubble in what should be a linear run of focused commits. If git refuses to fast-forward, something unexpected moved the task branch — stop and report it instead of reaching for a merge commit or a rebase.
    - Merging into the branch checked out in *this* worktree doesn't disturb the impl worktree, so the order above is safe: ff-merge first, then remove.
    - **Then remove the impl worktree.** It was only Codex's edit sandbox and its commits now live on the task branch, so it has served its purpose; leaving it registered clutters `git worktree list` and invites a stale-worktree mixup on the next accept run. Only remove a clean worktree — never pass `--force`; a refusal means something is still uncommitted, so go back to the loop and finish or drop it first.
    - **Then delete `<task>-impl`** — after the removal, not before: git refuses to delete a branch that any worktree still has checked out, ff-merged or not. Use `-d`, not `-D`: it succeeds only because the ff-merge already moved the commits onto the task branch, which is exactly the check you want.
    - This collapses the two-level stack down to the task branch, which now holds every reviewed commit and is the single thing the operator reviews and PRs. Say so when reporting.

11. **Close out the task worktree.** The task branch and its worktree are handled per CLAUDE.md — not by this skill. Do NOT push or merge to the integration branch: that still needs the operator's explicit instruction, per this project's git safety rules. Say explicitly if the task worktree is staying open for follow-up. Before starting a new accept run, check `git worktree list` / `git branch -a` for stale or overlapping worktrees and branches left over from earlier runs on this same task.
